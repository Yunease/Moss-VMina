#!/usr/bin/env python3
"""
Qwen3.5-2B QLoRA SFT Training (Fixed v4)

在 v3 基础上整合的改进:
1. [显式化] prompt 本身 >= max_length 时，不再靠 answer_len 被动 clamp 成 0 来"发现"
   问题，而是在加载阶段就显式判断 prefix_len >= max_length，直接跳过该样本并打印，
   原因和效果与 v3 等价，只是可读性和可诊断性更好。
2. [新增] 训练开始前对 train/val 全量样本做一次"坏样本扫描"：检查是否存在
   labels 全为 -100 的样本（正常情况下不应该有，因为 v3/v4 的 Dataset 已经在
   加载阶段过滤掉了；这一步是双重保险，防止未来改代码时又引入这个 bug）。
3. 仍然保留 v3 的核心修复：单次 tokenize 全量对话 + 在 token 序列里定位
   "<|im_start|>assistant\n" marker 来切分 prompt/answer 边界，而不是用两次
   独立的 apply_chat_template 调用做长度差（后者在 chat template 边界处的空白符/
   换行处理不一致时会导致边界错位，是你之前在 0.8B 上遇到过的同类问题）。
"""
