#!/usr/bin/env python3
"""
Moss VMina — SFT Data Preprocessing Pipeline (Sampling Edition)

Same logic as sft_pipeline.py but limited to N API calls (default 20)
so you can spot-check output quality without burning quota.

Usage:
    python sft_pipeline_sample.py                          # 20 calls, ./processed_sft_sample
    python sft_pipeline_sample.py --max-calls 50
    python sft_pipeline_sample.py --max-calls 10 --output-dir my_check
"""
...
log = logging.getLogger("sft_pipeline_sample")
...
class PipelineConfig:
    ...
    output_dir: str = "processed_sft_sample"
    checkpoint_db: str = ".sft_checkpoint_sample.db"

    # ---------- Data split ----------
    val_split: float = 0.1

    # ---------- API ----------
    api_base_url: str = "https://api.scnet.cn/api/llm/v1"
    api_key: str = ""
    ...
    # ---------- Sampling ----------
    max_calls: int = 20  # ← only this many API calls
...
def discover_schema(input_glob: str = "*.jsonl") -> SchemaReport:
    ...
    header = "=" * 62
    print(f"\n{header}")
    print("  STAGE 1 — SCHEMA DISCOVERY REPORT  (SAMPLING MODE)")
    print(f"{header}")
    for fp in files:
...
def run_pipeline(config: Optional[PipelineConfig] = None):
    ...
    skipped = 0
    items_to_process: list[tuple[Path, dict[str, Any], bool]] = []
    for fp, obj in all_items:
        ...
        if checkpoint.is_processed(item_id, id_chunk, source_name):
            skipped += 1
            continue
        is_val = random.random() < config.val_split
        items_to_process.append((fp, obj, is_val))

    # --- CROP to max_calls ---
    total_available = len(items_to_process)
    items_to_process = items_to_process[:config.max_calls]
    cropped = max(0, total_available - config.max_calls)

    already_done = checkpoint.count_processed()
    log.info(
        "Checkpoint: %d already processed, processing %d items (cropped %d, %d remain unprocessed)",
        already_done, len(items_to_process), cropped, total_available,
    )

    if not items_to_process:
        log.info("No items to process. Nothing to do.")
        checkpoint.close()
        return

    log.info("Processing items with dual-dice routing...")
    progress = tqdm.tqdm(
    ...
    for fp, obj, is_val in items_to_process:
        ...
        progress.set_postfix(
            ...
            train=train_count, val=val_count, refresh=False,
        )
        progress.update(1)
    ...
    print()
    print("=" * 62)
    print("  PIPELINE COMPLETE  (SAMPLING MODE)")
    print("=" * 62)
    print(f"  Total items in corpus:     {len(all_items)}")
    print(f"  Skipped (already done):    {skipped}")
    print(f"  Cropped (over max_calls):  {cropped}")
...
def parse_args(argv: list[str] | None = None) -> PipelineConfig:
    ...
    p = argparse.ArgumentParser(
        description="SFT Pipeline SAMPLING EDITION — limit to N calls for quality spot-check.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--input-glob", default="*.jsonl")
    p.add_argument("--output-dir", default="processed_sft_sample")
    p.add_argument("--checkpoint-db", default=".sft_checkpoint_sample.db")
    p.add_argument("--val-split", type=float, default=0.1)
    p.add_argument("--api-base-url", default="https://api.scnet.cn/api/llm/v1")
    p.add_argument("--api-model", default="DeepSeek-V4-Flash")
    p.add_argument("--api-key", default="")
    p.add_argument("--api-max-tokens", type=int, default=8192)
    p.add_argument("--api-temperature", type=float, default=0.7)
    p.add_argument("--max-rpm", type=int, default=30)
    p.add_argument("--max-calls", type=int, default=20, help="Only make this many API calls")
    ...
    cfg = PipelineConfig(
        ...
        api_key=args.api_key or os.environ.get("SFT_API_KEY") or cfg.api_key,
