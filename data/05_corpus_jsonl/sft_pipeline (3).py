...
class PipelineConfig:
    ...
    # ---------- API ----------
    # MiMo 开放平台，OpenAI 兼容端点（注意：MiMo-V2 系列已于 2026-06-30 下线，
    # 请使用 V2.5 系列模型名，如 mimo-v2.5 / mimo-v2.5-pro）
    api_base_url: str = "https://api.scnet.cn/api/llm/v1"
    api_key: str = ""  # set via env var SFT_API_KEY
    api_model: str = "DeepSeek-V4-Flash"
    api_timeout: float = 120.0
    api_max_retries: int = 5
    api_max_tokens: int = 6144
    api_temperature: float = 0.7
    # 默认禁用 thinking 模式，否则模型会先输出思考过程干扰 JSON 解析
    api_extra_body: dict[str, Any] | None = field(
...
def parse_args(argv: list[str] | None = None) -> PipelineConfig:
    ...
    p.add_argument("--api-base-url", default="https://api.xiaomimimo.com/v1", help="API base URL")
    p.add_argument("--api-model", default="mimo-v2.5", help="Model name (mimo-v2.5 / mimo-v2.5-pro)")
